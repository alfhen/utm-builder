...
const showTrafficToggle = computed(() => {
  ...
  const hasPaid = platformChannels.some(c => c.trafficType === 'paid' || c.trafficType === 'paid_search');
  const hasOrganic = platformChannels.some(c => c.trafficType === 'organic' || c.trafficType === 'organic_social');
...
function handleTrafficTypeChange(isPaid: boolean) {
  ...
  const newChannel = channels.find(
    c => c.platform === currentPlatform.value && 
         (c.trafficType === targetType || c.trafficType === `${targetType}_search` || c.trafficType === `${targetType}_social`)
  );
    ...
    <div class="max-w-2xl mx-auto">
      ...
      <header class="text-center mb-10">
        <h1
          ...
          style="font-family: 'Syne', sans-serif; background: linear-gradient(135deg, #00d4ff 0%, #c4ff00 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;"
        >
          UTM Builder
        </h1>
      ...
      <main class="bg-surface-800 rounded-2xl border border-surface-600 p-6 sm:p-8 shadow-2xl">
        ...
        <div v-if="validationResult" class="mt-8 pt-8 border-t border-surface-600">
          <ValidationResults :result="validationResult" @fix="handleFix" @fix-all="handleFixAll" />
        </div>
      </main>
      ...
      <footer class="mt-8 text-center text-zinc-500 text-sm">
        <p>
          Build consistent UTM parameters for your marketing campaigns.
        </p>
      </footer>
    </div>
  </div>
